...
def test_c_proj_forward_backward():
    torch.manual_seed(0)
    x = torch.randn(2, 2, 4, 4, device="cuda", requires_grad=True)
    ...
    ref_out = ref_c_proj(x, weight, bias)
    triton_out = triton_c_proj(*triton_args)
